...
use Siplec\CdBundle\Form\ReexpeditionType;
...
class ReexpeditionController extends Controller
{
     ...
     * @Method("POST|GET")
     * @Template("SiplecCdBundle:Reexpedition:index.html.twig")
     */
    public function indexAction(Request $request)
    {
        $em = $this->getDoctrine()->getManager();
        // Récupération des différents statut utilisé pour le traitement des réexpédition
        $statutRefuse = $em->getRepository('SiplecCdBundle:StatutDossier')->find(Dossier::STATUT_REFUSE);
        $statutRefuseAuto = $em->getRepository('SiplecCdBundle:StatutDossier')->find(Dossier::STATUT_REFUSE_AUTOMATIQUE);
        $statutValide = $em->getRepository('SiplecCdBundle:StatutDossier')->find(Dossier::STATUT_VALIDE);
        $dossier = new Dossier();
        	...
        	if ($dossierSelection) {
	        	...
	        	if(in_array($dossierSelection->getStatut(), array($statutValide, $statutRefuse, $statutRefuseAuto))){
	        		...
	        		if($dossierSelection->getDateReexpedition() == null && $dossierSelection->getRefRecall() == null){
	        			...
	        			$em->getRepository('SiplecCdBundle:Dossier')->setReexpeditionFamily($dossier, $this->getUser());
    ...
    */
    ...
    public function ajaxAction(){
    	...
    	$count = $em->getRepository('SiplecCdBundle:Dossier')->getNbResultatsReexpedition($criteria, $this->getUser());
    	$entities = $em->getRepository('SiplecCdBundle:Dossier')->getSearchReexpedition($criteria, $this->getUser(), array($sidx=>$sord), $limit, $page);
