...
use Siplec\CdBundle\Form\TraitementType;
...
class TraitementController extends Controller
{
    ...
    private function logiqueTraitementFirstEtape(Dossier $dossier)
    {
		...
		$dossierSelection = $em->getRepository('SiplecCdBundle:Dossier')->getFirstDossierByStatutRecuOrEnCours($dossier);
    ...
    private function logiqueTraitementSecondEtape(Dossier $dossier)
    {
    	...
    	switch($dossier->getStatut()->getId())
    	{
    		...
    		case Dossier::STATUT_INCOMPLET:
    			switch($dossier->getTypeDossier())
    			{
    				case Dossier::TYPE_DOSSIER_NOUVEAU:
    					$archive = $em->getRepository('SiplecCdBundle:Archive')->getFirstLineLibre();
    					if($archive == null)
    					{
    		...
    		case Dossier::STATUT_REFUSE:
   				$em->getRepository('SiplecCdBundle:Dossier')->libereArchive($dossier);
    ...
    private function setTraitementStatutEnCours(Dossier $dossier)
    {
    	...
    	$statut = $em->getRepository('SiplecCdBundle:StatutDossier')->find(Dossier::STATUT_EN_COURS_DE_TRAITEMENT);
