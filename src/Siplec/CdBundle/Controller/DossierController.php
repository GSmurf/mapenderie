...
use Siplec\CdBundle\Entity\Archive;
use Siplec\CdBundle\Form\DossierType;
use Siplec\CdBundle\Form\ImportDossierType;
...
class DossierController extends Controller
{
    ...
    public function indexAction(Request $request)
    {
        ...
        $entities = $em->getRepository('SiplecCdBundle:Dossier')->findAll();
        $thematiques = $em->getRepository('SiplecCdBundle:Thematique')->findAll();
        $statutDossier = $em->getRepository('SiplecCdBundle:StatutDossier')->findAll();
     ...
     * @Method("POST")
     * @Template("SiplecCdBundle:Dossier:new.html.twig")
     */
    public function createAction(Request $request)
    {
        ...
        if ($form->isValid()) {
            ...
            $statut = $em->getRepository('SiplecCdBundle:StatutDossier')->find(Dossier::STATUT_RECU);
            ...
            $em->getRepository('SiplecCdBundle:Courrier')->dossierAdequationReçu($dossier);
    ...
    */
    ...
    public function newAction()
    {
        ...
        if($this->getRequest()->getSession()->get('thematique')){
        	$dossier->setThematique($em->getRepository('SiplecCdBundle:Thematique')
    	...
    	return $this->render(
    			'SiplecCdBundle:Dossier:index.html.twig',
    			array(
		            'entity' => $dossier,
	        		'thematiques' => $thematiques,
	        		'statutDossier' => $statutDossier,
    ...
    public function showAction($id)
    {
        ...
        $entity = $em->getRepository('SiplecCdBundle:Dossier')->find($id);
    ...
    */
     ...
     * @Method("PUT")
     * @Template("SiplecCdBundle:Dossier:edit.html.twig")
     */
    public function updateAction(Request $request, $id)
    {
        $em = $this->getDoctrine()->getManager();

        $dossier = $em->getRepository('SiplecCdBundle:Dossier')->find($id);
        if (!$dossier) {
            throw $this->createNotFoundException('Unable to find Dossier entity.');
        }
    ...
    public function deleteAction(Request $request, $id)
    {
			...
			{
				$em->getRepository('SiplecCdBundle:Dossier')->libereArchive($entity);
			}
            $em->remove($entity);
            $em->flush();
    ...
    public function ajaxGetDossierAction($saisie){
    	...
    	if($saisie == 1)
    	{
    		$count = $em->getRepository('SiplecCdBundle:Dossier')->getNbResultatsDossier($criteria, $this->getUser());
    		$entities = $em->getRepository('SiplecCdBundle:Dossier')->getSearchDossier($criteria, $this->getUser(), array($sidx=>$sord), $limit, $page);
    	}else{
    		$count = $em->getRepository('SiplecCdBundle:Dossier')->getNbResultats($criteria, array($sidx=>$sord));
    		$entities = $em->getRepository('SiplecCdBundle:Dossier')->getSearch($criteria, array($sidx=>$sord), $limit, $page);
    	}
    	if( $count >0 ) {
    ...
    private function logiqueDossier(Dossier $dossier){
    	...
    	$rechercheDossierExistant = $em->getRepository('SiplecCdBundle:Dossier')->findOneBy(array('numDossier' => $dossier->getNumDossier(), 'typeDossier' => Dossier::TYPE_DOSSIER_NOUVEAU));
    	...
    	if($dossier->getTypeDossier() == Dossier::TYPE_DOSSIER_COMPLEMENT){
	    	...
	    	$dossierPere = $em->getRepository('SiplecCdBundle:Dossier')->getPere($dossier);
	    	if ($dossierPere != null) {
    ...
    public function ExportDossierATraiterAction(Request $request){
    	...
    	$em = $this->getDoctrine()->getManager();
    	$results = $em->getRepository('SiplecCdBundle:Dossier')->getATraiter();
    ...
    public function importAction(Request $request)
    {
        ...
        if ($form->isValid()) {
        	...
        	if (($handle = fopen ( $dir.$nomFichier, "r" )) !== FALSE) {
        		$em = $this->getDoctrine()->getManager();
        		$maxArchive = 0;
        		$thematique = $em->getRepository('SiplecCdBundle:Thematique')->find(DOSSIER::THEMATIQUE_INITIALISATION);
        		...
        		foreach ($data as $row) {
					$archiveImport = $em->getRepository('SiplecCdBundle:Archive')->find($row['archive']);
					...
					$dossierImport->setArchive($archiveImport)
								...
								->setStatut($em->getRepository('SiplecCdBundle:StatutDossier')->find(DOSSIER::STATUT_INCOMPLET))
