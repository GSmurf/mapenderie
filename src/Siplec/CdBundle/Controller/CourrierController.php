...
use Siplec\CdBundle\Entity\Courrier;
...
class CourrierController extends Controller
{
    ...
    {
    	return $this->render(
    			'SiplecCdBundle:Courrier:index.html.twig',
    			array('saisie'=>true,
        			'saisieUrl'=>1,
    ...
    public function ExportNonReconnusAction(Request $request){
    	...
    	$results = $em->getRepository('SiplecCdBundle:Courrier')->getNonReconnus();
    ...
    public function ajaxGetCourriersAction($saisie){
        ...
        $em = $this->getDoctrine()->getManager();
    	$count = $em->getRepository('SiplecCdBundle:Courrier')->getNbResultats($criteria, array($sidx=>$sord));
        $entities = $em->getRepository('SiplecCdBundle:Courrier')->getSearch($criteria, array($sidx=>$sord), $limit, $page);
    	if( $count >0 ) {
    ...
    public function ajaxUpdateCourriersAction(Request $request){
    	$new = false;
        $em = $this->getDoctrine()->getManager();
        $courrier = $em->getRepository('SiplecCdBundle:Courrier')->find($_POST['id']);
    ...
    private function logiqueCourrier(Courrier &$courrier){
    	...
    	$dossier = $em->getRepository('SiplecCdBundle:Dossier')->getLastDossierByTypeCourrierAndNumDossier($courrier->getNumDossier(), $courrier->getTypeEnvoi());
    	if($dossier){
