...
use Penderie\DefaultBundle\Form\RefusAutomatiqueType;
use Symfony\Component\HttpFoundation\JsonResponse;

/**
 * RefusAutomatique controller.
 * TODO : extends ControllerAdéquation qui serai dans le namespace Penderie\DefaultBundle\Controller qui permettrai de définir au moins deux méthodes : une d'export en excel generique qui attends ($tableauxResultat, $titre, $nomFichier) et la fonction de transformeDate 
 *
 * @Route("/refus-auto")
 */
class RefusAutomatiqueController extends Controller
{
     ...
     * @Method("POST|GET")
     * @Template("PenderieDefaultBundle:RefusAutomatique:index.html.twig")
     */
    public function indexAction(Request $request, $import)
    {
        $em = $this->getDoctrine()->getManager();
        $statutRefuseEnCours = $em->getRepository('PenderieDefaultBundle:StatutDossier')->find(Dossier::STATUT_REFUS_AUTOMATIQUE_EN_COURS);
        ...
        if ($formImport->isValid()) {
        	...
        	if (($handle = fopen ( $dir.$nomFichier, "r" )) !== FALSE) {
				while ( ($data = fgetcsv ( $handle, 1000, "," )) !== FALSE ) {
					...
					$num = count ( $data );
					for($c = 0; $c < $num; $c ++) {
						$dossier = $em->getRepository('PenderieDefaultBundle:Dossier')->getLastDossierByNumDossierAndStatutIncomplet($data[$c]);
        	...
        	if ($dossierSelection) {
	        	if(in_array($dossierSelection->getStatut(), array($statutRefuseEnCours, $statutRefuse))){
		        	if($dossierSelection->getDateRefus() == null){
			        	...
			        	$em->getRepository('PenderieDefaultBundle:Dossier')->libereArchive($dossierSelection);
			        	$em->flush();
